...
def run_vis_on_demo(cfg, video, results, output_pth, smpl, vis_global=True):
    ...
    if vis_global:
        ...
        for k in results.keys():
            global_output = smpl.get_output(
                ...
                tt(results[k]['trans'][[0]]))
            verts_glob = global_output.vertices.cpu()
            yup2ydown = axis_angle_to_matrix(torch.tensor(
                [[0, 0, np.pi]])).float().to(verts_glob.device)
            verts_glob = verts_glob @ yup2ydown
            # verts_glob[..., 1] = verts_glob[..., 1] - verts_glob[..., 1].min()
            results[k]['verts_glob'] = verts_glob

        n_frames = {k: len(results[k]['frame_ids'])}
        ...
        # set default ground, hardcoded
        renderer.set_ground(30, 0, 10)

        # build global camera
        global_R, global_T, global_lights = get_global_cameras(
            verts_glob, cfg.DEVICE)

    # build default camera
    default_R, default_T = torch.eye(3), torch.zeros(3)

    default_R_2, default_T_2 = torch.eye(3), torch.zeros(3)
    default_T_2[1] = -10
    default_T_2[2] = 30
    default_R_2 = axis_angle_to_matrix(torch.tensor([[-np.pi/2, 0, 0]]))
    ...
    while (cap.isOpened()):
        ...
        if vis_global:
            ...
            all_colors = []
            for k in results.keys():
                if frame_i in results[k]['frame_ids']:
                    frame_i3 = np.where(results[k]['frame_ids'] == frame_i)[0]
                    verts = results[k]['verts_glob'][[frame_i3]].to(cfg.DEVICE)
                    faces = renderer.faces.clone().squeeze(0)
                    colors = torch.ones((1, 4)).float().to(cfg.DEVICE)
                    colors[..., :3] = torch.from_numpy(global_colors[k])
            ...
            cameras = renderer.create_camera(default_R_2, default_T_2)
            img_glob = renderer.render_with_ground(verts, faces, colors,
                                                   cameras, global_lights)
